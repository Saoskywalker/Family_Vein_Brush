...
void MPU_IIC_Start(void)
{
	...
	MPU_IIC_Delay();
	MPU_IIC_SCL &= ~MPU_IIC_SCL_STATUS;//钳住I2C总线，准备发送或接收数据 
}	  
//产生IIC停止信号
void MPU_IIC_Stop(void)
{
	...
	MPU_IIC_SDA &= ~MPU_IIC_SDA_STATUS;//STOP:when CLK is high DATA change form low to high
 	MPU_IIC_Delay();
	MPU_IIC_SCL |= MPU_IIC_SCL_STATUS; 
	MPU_IIC_SDA |= MPU_IIC_SDA_STATUS;//发送I2C总线结束信号
	MPU_IIC_Delay();							   	
}
//等待应答信号到来
//返回值：1，接收应答失败
...
u8 MPU_IIC_Wait_Ack(void)
{
	...
	MPU_IIC_SCL &= ~MPU_IIC_SCL_STATUS;//时钟输出0 	   
	return 0;  
} 
//产生ACK应答
...
{                        
    u8 t;   
	MPU_SDA_OUT(); 	    
    MPU_IIC_SCL &= ~MPU_IIC_SCL_STATUS;//拉低时钟开始数据传输
    for(t=0;t<8;t++)
    {              
       if ((txd&0x80)>>7)
       {
